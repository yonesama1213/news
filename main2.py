...
def summarize_with_retry(title, max_retries=3):
    # 最新モデルを指定
    model_names = ["gemini-2.5-flash", "models/gemini-1.5-flash"]
    prompt = f"""
   あなたは親しみやすい高校教師です。以下のニュースを、授業の合間に生徒に教えるようなイメージで解説してください。
    
    【制約事項】
    1. 専門用語を避け、日常的な言葉（例：『互換性』→『使い回しができること』）を使ってください。
    2. 結論から先に述べ、なぜそれが重要なのかを10代の視点で説明してください。
    3. 全体で3文程度の日本語で要約してください。
    4. 難しい言葉を2つ選び、中学生でもわかるレベルで解説（glossary）してください。
